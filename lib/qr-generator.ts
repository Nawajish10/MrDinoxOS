...
// Generate table menu QR code for QR code ordering system
export async function generateTableMenuQR(restaurantId: string, tableId: string): Promise<string> {
    const appUrl = typeof window !== 'undefined'
        ? window.location.origin
        : process.env.NEXT_PUBLIC_APP_URL || ''
    const menuUrl = `${appUrl}/customer/menu?type=dine_in&restaurant=${restaurantId}&table=${tableId}`
    return generateQRDataURL(menuUrl, { width: 400 })
}

// Generate table QR code (legacy - for WhatsApp)
